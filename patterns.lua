...
SYSLOGBASE2 = "({:timestamp:SYSLOGTIMESTAMP:} / {:timestamp8601:TIMESTAMPISO8601:}) ' ' (SYSLOGFACILITY ' ')? {:logsource:SYSLOGHOST:} ' ' SYSLOGPROG ':'"
SYSLOGLINE = "SYSLOGBASE2 ' ' {:message:GREEDYDATA:}"
SYSLOG5424PRINTASCII = "[%p%w]+"
SYSLOG5424PRI = "'<' {:syslog5424_pri:NONNEGINT:} '>'"
SYSLOG5424SDNAME = "%w+ ('@' %w+)?"
SYSLOG5424SDPARAMS = [[(' ' {| {:name:%w+:} '="' {:value:%w+:} |} '"')*]]
SYSLOG5424SD = "{| ('[' {| {:id:SYSLOG5424SDNAME:} {:params:{|SYSLOG5424SDPARAMS|}:} |} ']')* |}"
SYSLOG5424BASE = "SYSLOG5424PRI {:syslog5424_ver:NONNEGINT:} ' '+ ( {:syslog5424_ts:TIMESTAMPISO8601:} / '-') ' '+ ( {:syslog5424_host:HOSTNAME:} / '-') ' '+ ('-' / {:syslog5424_app:SYSLOG5424PRINTASCII:}) ' '+ ('-' / {:syslog5424_proc:SYSLOG5424PRINTASCII:}) ' '+ ('-' / {:syslog5424_msgid:SYSLOG5424PRINTASCII:}) ' '? ('-' / {:syslog5424_sd:SYSLOG5424SD:})?"
SYSLOG5424LINE = "SYSLOG5424BASE ' '+ {:syslog5424_msg:GREEDYDATA:}"
